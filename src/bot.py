...
# ------------------------------------------------------------------
TEST_MODE = False  # True — тестовый режим, False — боевой с Ollama
...
async def on_ready():
    ...
    await bot.tree.sync()
    mode_str = f"{YELLOW}[ТЕСТОВЫЙ РЕЖИМ]{RESET}" if TEST_MODE else f"{GREEN}[БОЕВОЙ РЕЖИМ]{RESET}"
