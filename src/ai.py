...
# ------------------------------------------------------------------
# ВСПОМОГАТЕЛЬНЫЕ ФУНКЦИИ (КОНФИГУРАЦИЯ И БАНЫ)
# ------------------------------------------------------------------
def get_whitelist() -> list:
    ...
    try:
        with open(config_file, "r", encoding="utf-8") as f:
            data = json.load(f)
            return [int(x) for x in data.get("whitelist", [])]
    except Exception as e:
...
def get_ollama_response(prompt: str):
    ...
    if TEST_MODE:
        ...
        return f"\n```тестовое сообщение```\n\nВремя генерации ответа: `{gen_time}` сек"
...
# ------------------------------------------------------------------
# РЕГИСТРАЦИЯ КОМАНД В БОТА (Вызывается один раз из main.py)
# ------------------------------------------------------------------
